...
export const generateCSVContent = (bills: Bill[]): string => {
  ...
  const rows = bills.map((bill) => {
    ...
    // Calculate days
    const days = getDaysDifference(bill.checkInDate, bill.checkOutDate);

    // Extract date & month
    let monthStr = '';
    let dateStr = bill.checkOutDate || '';
    if (dateStr) {
      ...
      monthStr = `${d.getMonth() + 1}月`;
      dateStr = bill.createdAt.slice(0, 10);
    }

    // Category quantities & amounts
    let qtyDaycare = 0;
    let amtDaycare = 0;
    ...
    bill.items.forEach((item) => {
      ...
      const category = item.name.includes('牙结石') ? '洁牙' : (item.name.includes('游泳') ? '游泳' : (item.name.includes('加班') || item.name.includes('加班费') ? '加班费' : ''));
      
      const qty = item.quantity;
      const discount = bill.discount ?? 1.0;
      
      // We check category or name keyword
      if (name.includes('牙结石') || name.includes('洁牙')) {
        ...
        // Standard categories
        const srvCategory = item.serviceId ? 'unknown' : 'unknown'; // fallback or check from srv?
        // Wait, we can find standard categories based on item.name or pre-mapped structure, but we also have categories on DEFAULT_SERVICES.
        // Actually, we can check a map of item.name or subCategory clues:
        if (name.includes('日托')) {
          qtyDaycare += qty;
